...
for (let i = 0; i < dataArray.length; i++) {
	...
	const checksum = line.substring(line.length - 6, line.length - 1);
	// console.log(">>> working with :: checksum", checksum);
	const sectorID = line.substring(line.length - 10, line.length - 7);
	// console.log(">>> working with :: sectorID", sectorID);
	const name = line.substring(0, line.length - 11).replace(/-/g, '');
	// console.log(">>> working with :: name", name);

	const amountOfChar = [0, 0, 0, 0, 0];

	for (let i = 0; i < name.length; i++) {
		const char = name[i];

		if (checksum.indexOf(char) > -1) {
			amountOfChar[checksum.indexOf(char)] += 1;
		}
	}
	console.log(">>> working with :: amountOfChar", amountOfChar);

	let valid = true;

	if (amountOfChar.indexOf(0) > -1) {
		console.log(">>> a zero was found!!! invalid checksum");
		valid = false;
	}

	for (let j = 0; j < amountOfChar.length - 1; j++) {
		if (!valid) break;
		// console.log(">>> working with :: ["+j+"] amountOfChar ["+amountOfChar[j]+"] - j+1 ["+amountOfChar[j+1]+"]");
		if (amountOfChar[j] < amountOfChar[j + 1]) {
			valid = false;
			break;
		}
	}
	...
	if (valid) {
		...
		console.log(">>> Checksum good!! adding ["+sectorID+"] to sum", sum);
	}
		console.log("---------------------------------------------------------------");

}

console.log(`PART ONE: The sum of all real sector IDs is: ${sum}`);
