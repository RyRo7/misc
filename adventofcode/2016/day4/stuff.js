...
function part1(){
		...
		console.log('What is the sum of the sector IDs of the real rooms?', answer);
};
...
function part2(){
		...
		var decrypt = "";
		rooms.forEach(function(room) {
			var isValid = validRoomPart2(room);
			if(isValid.status) {
				// console.log('decrypt '+isValid.room);
				decrypt = caesarShift(isValid.room, isValid.sector).replace(/-/g,' ');
				// console.log(isValid.sector + ': '+caesarShift(isValid.room, isValid.sector).replace(/-/g,' '));
				if (decrypt.toLowerCase().indexOf("northpole") > -1) {
					answer = isValid.sector;
				}
			}
		});

		console.log('What is the sector ID of the room where North Pole objects are stored?', answer);
};

/*
if true, returns sector, otherwise returns 0
*/
function validRoomPart2(str) {
//    console.log('input = '+str);
    var parts = str.split('-');
    //first, get the room, which is all but the last
    var room = parts.slice(0, parts.length - 1).join('-');
